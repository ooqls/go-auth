...
func (a *AuthenticationServerImpl) LoginChallengeResponse(ctx *gin.Context) {
	...
	ctx.SetCookie("OKEY", result.Okey, 3600, "/", host, a.IsSecure, true)
	ctx.SetCookie("RKEY", result.Rkey, 3600, "/", host, a.IsSecure, true)
	ctx.SetCookie("UID", result.UserID, 3600, "/", host, a.IsSecure, true)
...
func (a *AuthenticationServerImpl) IsAuthed(ctx *gin.Context) {
	...
	ctx.Writer.Header().Add("X-User-Id", claims.UserID.String())
	ctx.Status(200)
}
