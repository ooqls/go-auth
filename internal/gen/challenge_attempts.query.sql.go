...
func (q *Queries) GetChallengeAttempts(ctx context.Context, userID uuid.UUID) ([]Challengeattemptsv1, error) {
	rows, err := q.db.QueryContext(ctx, getChallengeAttempts, userID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []Challengeattemptsv1
	for rows.Next() {
		var i Challengeattemptsv1
		if err := rows.Scan(
			&i.ID,
			&i.ChallengeID,
			&i.UserID,
...
func (q *Queries) GetFailedAttempts(ctx context.Context, arg GetFailedAttemptsParams) ([]Challengeattemptsv1, error) {
